...
def getDiffList(argv_start,argv_end,argv_line):
    temp_list = list()
    cmd_text = 'svn diff --summarize -r %d:%d "svn://192.168.2.6/parkout_conf/%s/assets/pic/res" > diff.txt'%(argv_start,argv_end,argv_line)
    if argv_end == 0:
        cmd_text = 'svn diff --summarize -r %d "svn://192.168.2.6/parkout_conf/%s/assets/pic/res" > diff.txt'%(argv_start,argv_line)
...
def getImageFlag(img_name):
    ...
    if img_name.find('\\csb\\world') >= 0 :
        temp_flag = IMAGE_FLAG_MAP

    if img_name.find('\\fonts\\') >= 0 :
        temp_flag = IMAGE_FLAG_FONT
...
def renameImg():
    path_dict = {}
    walkpath("..\\..\\outputClient",path_dict)
    ...
    for row_key,row_value in path_dict.iteritems():
        ...
        for j in xrange(0,len(row_value)):
            ...
            if img_name.find('.pvr.ccz') >= 0:
                png_name = img_name.replace("pvr.ccz","png")
                os.rename(img_name,png_name)
            elif img_name.find('.plist') >= 0:
                #转换所有pvr.ccz为png
                with open(img_name, 'r') as fr:
                    content = fr.read()
                content = content.replace(".pvr.ccz</string>",".png</string>")
                safeWrite(img_name,content)
