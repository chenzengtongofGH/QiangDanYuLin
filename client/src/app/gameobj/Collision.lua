...
--检测英雄跟物体碰撞, 碰撞检测中不能设置gameobj的位置，否则出错
function MCollision.checkHeroObjCollision( objhero, objtable, collList, dt, IsBouns)
	-- print("========================================")
	local herorect = objhero:getRect()
	local herooldpos = objhero:get_oldpos()

	for k,v in pairs( objtable ) do
		local trect = v:getRect()

		local touched = cc.rectIntersectsRect( herorect, trect )
		if touched then
			-- print("touched")
			local collitem = {}
			collitem.obj = v
			if v:isCollisionBlock() then
				if  IsBouns then
					if v:getType()==GAMEBLOCK then
				-- 	if v:getType()==MGameEnum.kObjBlock and v:getIsBouns()==true then
				-- 		-- print("MCollision.checkHeroObjCollision",IsBouns , v:getIsBouns())
				-- 		-- if v:getIsBouns()==true then
				-- 			-- print(" 超级奖励的地板块 ")
							local collstat = MCollision.getCollisionSide( herorect, trect, herooldpos, v:get_oldpos(), dt)
							collitem.collstat = collstat
				-- 			table.insert(collList, collitem)
				-- 			print("这是超级奖励和地板快碰撞的物理块",IsBouns,#collList,v:getIsBouns(),collstat.bottom,collstat.blockbottom)
				-- 			-- print(json.encode(collstat))
			...
			-- print(#collList)
			table.insert(collList, collitem)
            print("Hero_PosX:"..herorect.x);
            print("Trect_PosX:"..trect.x);
			-- print(#collList)
			v:redrawRect()
		end
	end
