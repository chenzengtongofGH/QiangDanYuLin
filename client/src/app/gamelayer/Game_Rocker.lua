...
function Game_Rocker:init()
    ...
    rocker_rad = rock_sprite:getContentSize().width/2  - 5;
...
function Game_Rocker:updateVelocity(point)
    local dx = point.x;
    local dy = point.y;
    if dx > rocker_rad then 
        dx = rocker_rad;
    end
    if dy > rocker_rad then 
        dy = rocker_rad;
    end
    if dx<-rocker_rad then 
        dx = -rocker_rad;
    ...
    local temp_rad =   math.sqrt(dSq)
    if temp_rad > self.joystickRadius then 
        local scale = temp_rad / self.joystickRadius;
        dx = dx / scale;
        dy = dy / scale;
    end
 ...
 --   if dSq > self.joystickRadiusSq or self.isDPad then 
    ...
    --print("updateVelocity.dx:"..dx..",dy:"..dy..",angle:"..angle..",sinAngle:"..sinAngle..",cosAngle:"..cosAngle);
    self.point_sprite:setPosition(stickPosition);
    --set_Map_Move
